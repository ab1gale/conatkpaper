...
%Some concurrent bugs cause execution order violation, wrong output, program crash. 
...
% First Most concurrent attacks is due to data race

% Second, concurrent attacks may happen after raced variables propagating through program executions. 
%(Through control flow and data flow). 

% Third, bugs that cause crash may lead to worse effects..


%Although extant work is great, they cannot solve the following issues.

%First, concurrent attacks are hidden in large number of bug reports. Exsiting works cannot provide 
%analysis on these attacks. 
